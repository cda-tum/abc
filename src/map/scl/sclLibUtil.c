...
//    Abc_SclMarkSkippedCells( p );
    ...
    Vec_StrPrintStr( vStr, "GATE _const0_      0.00     0.00       0.00 z=CONST0;\n" );
    Vec_StrPrintStr( vStr, "GATE _const1_      0.00     0.00       0.00 z=CONST1;\n" );
    SC_LibForEachCell( p, pCell, i )
    {
        if ( pCell->n_inputs == 0 )
            continue;
        ...
        SC_CellForEachPinOut( pCell, pPinOut, j )
        {
            ...
            Vec_StrPrintStr( vStr, Buffer );
            Vec_StrPrintStr( vStr, " " );
            float CellInternalPower = Abc_SclComputeAverageNetSwitchingPower( &pCell );
            sprintf( Buffer, "%f \n", CellInternalPower );
            Vec_StrPrintStr( vStr, Buffer );
            Vec_StrPrintStr( vStr, " " );
            float NetSwitchingPower = Abc_SclComputeAverageCellInternalPower( &pCell );
            sprintf( Buffer, "%f \n", NetSwitchingPower );
            Vec_StrPrintStr( vStr, Buffer );
            Vec_StrPrintStr( vStr, " " );
...
//        sprintf( Buffer, "%7.2f", Abc_SclComputeAreaClass(pRepr) );
        ...
        Vec_StrPrintStr( vStr, Buffer );
        Vec_StrPrintStr( vStr, " " );
        float CellInternalPower = Abc_SclComputeAverageCellInternalPower( &pRepr );
        sprintf( Buffer, "%f \n", CellInternalPower );
        Vec_StrPrintStr( vStr, Buffer );
        Vec_StrPrintStr( vStr, " " );
        float NetSwitchingPower = Abc_SclComputeAverageNetSwitchingPower( &pRepr );
        sprintf( Buffer, "%f \n", NetSwitchingPower );
        Vec_StrPrintStr( vStr, Buffer );
        Vec_StrPrintStr( vStr, " " );
