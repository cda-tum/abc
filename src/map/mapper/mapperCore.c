...
//ABC_PRT( "Truths", Abc_Clock() - clk );
    ...
    p->SwitchPowerBase = Map_MappingGetSwitchPower( p );
if ( p->fVerbose )
{
...
ABC_PRT( "Time", p->timeMatch );
    ...
    if ( fUsePowerFlow2 )
    {
        ...
        p->AreaFinal = Map_MappingGetArea( p );
        p->PowerFinal = Map_MappingGetPower( p );
        p->SwitchPowerFinal = Map_MappingGetSwitchPower( p );
        if ( p->fVerbose )
        {
            printf( "PowerFlow : %s = %8.2f  Flow = %11.1f  Area = %11.1f  %4.1f %%  DSPower = %11.1f  %4.1f %%  ",
                    fShowSwitching? "Switch" : "Delay",
                    fShowSwitching? Map_MappingGetSwitching(p) : p->fRequiredGlo,
                    Map_MappingGetAreaFlow(p), p->AreaFinal,
                    100.0*(p->AreaBase-p->AreaFinal)/p->AreaBase , p->SwitchPowerFinal, 100.0*(p->SwitchPowerBase-p->SwitchPowerFinal)/p->SwitchPowerBase  );
            ABC_PRT( "Time", Abc_Clock() - clk );
        }
    }
    p->timeArea += Abc_Clock() - clk;
